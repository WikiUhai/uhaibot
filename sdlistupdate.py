...
if len(list_entries) > 0:
    ...
    query = "SELECT p.page_title, CASE WHEN p.page_id IN (SELECT cl.cl_from FROM categorylinks cl WHERE cl.cl_to IN ('Articles_with_short_description', 'All_redirects_for_discussion')) OR p.page_is_redirect THEN 1 ELSE 0 END FROM page p WHERE p.page_namespace = 0 AND p.page_title IN (%s);" % in_subst
